...
class PatchMaskGenerator:
    def __init__(self, annotations_csv, image_dir, mask_dir, slide_path):
        ...
        self.patch_width = self._get_largest_factor_in_range(self.slide_width, 1600, 2200)
        self.patch_height = self._get_largest_factor_in_range(self.slide_height, 1600, 2200)
        ...
        # Changed to only include lobule with pure yellow color
        self.color_mapping = {
            'lobule': (1, 1, 0)  # Pure yellow (R=1, G=1, B=0)
        }
...
if __name__ == "__main__":
    generator = PatchMaskGenerator(
        annotations_csv=r"Annotationsv.csv",
        image_dir=r"images",
        mask_dir=r"masks",
        slide_path=r"WSI.ndpi"
    )
    generator.load_data()
